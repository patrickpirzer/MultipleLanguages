...
namespace MultipleLanguagesLogTranslate
{
    public class XmlLanguageDictionary : LanguageDictionary
    {
        ...
        public XmlLanguageDictionary(string path)
        {
            if (!File.Exists(path))
            {
                throw new ArgumentException(string.Format("The file {0} doesn't exist", path));
            }
            this._path = path;
        }
        ...
        protected override object OnTranslate(string uid, string vid, Type type)
        {
            ...
            {
                #region Trace
                Debug.WriteLine(string.Format("Failed to translate text {0} in dictionary {1}", textValue, EnglishName));
                #endregion
                return null;
            }
        }
